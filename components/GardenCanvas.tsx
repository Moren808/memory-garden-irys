...
export const GardenCanvas: React.FC<GardenCanvasProps> = ({ trees, onTreeSelect, onTreeMove, waterEventId }) => {
  ...
  useEffect(() => {
    ...
    // FIX: To handle clearing the garden, we must remove trees from our internal
    ...
    // FIX: Add explicit type for 'tree' to fix type inference issues.
  ...
  useEffect(() => {
    ...
    const getTreeAtPos = (x: number, y: number): AnimatedTree | undefined => {
        ...
        const treesArray = Array.from(animatedTreesRef.current.values()).reverse();
        // FIX: Add type assertion to fix type inference issue on iterated array.
        for (const tree of treesArray as AnimatedTree[]) {
    ...
    const drawBranch = (
      ...
      const style = {
        ...
        branchShadow: 'rgba(10, 12, 18, 0.5)',
      ...
      gradient.addColorStop(0, style.branchColor);
      gradient.addColorStop(1, style.branchShadow);
    ...
    const animate = () => {
      ...
      // FIX: Add explicit type to `treesToAnimate` to prevent properties on `tree` being of type `unknown`.
      const treesToAnimate: AnimatedTree[] = Array.from(animatedTreesRef.current.values());

      treesToAnimate.forEach(tree => {
        ...
        // Draw root mound
        const moundRadius = tree.currentGrowth * 4;
        const moundGradient = ctx.createRadialGradient(tree.x, tree.y, 1, tree.x, tree.y, moundRadius);
        moundGradient.addColorStop(0, 'rgba(80, 254, 213, 0.15)');
        moundGradient.addColorStop(1, 'rgba(80, 254, 213, 0)');
        ctx.fillStyle = moundGradient;
        ctx.beginPath();
        ctx.arc(tree.x, tree.y, moundRadius, 0, Math.PI * 2);
        ctx.fill();
        ...
        tree.particles.forEach(p => {
            ...
            const flutter = Math.sin(tree.swayPhase * 2 + p.angle) * 2;
            const x = canopyCenterX + Math.cos(p.angle) * orbitRadius;
            const y = canopyCenterY + Math.sin(p.angle) * orbitRadius * 0.6 + flutter;
            ctx.beginPath();
      ...
      for (const treeA of treesToAnimate) {
        if (treeA.currentGrowth < 2.5) continue;

        for (const treeB of treesToAnimate) {
          if (treeA.id >= treeB.id || treeB.currentGrowth < 2.5) continue;

          const canopyAY = treeA.y - treeA.currentGrowth * 30 * 0.7;
          const canopyBY = treeB.y - treeB.currentGrowth * 30 * 0.7;
          const dist = Math.hypot(treeA.x - treeB.x, canopyAY - canopyBY);
          const connectRadius = 280;

          if (dist < connectRadius) {
            const connectChance = (1 - dist / connectRadius) ** 2;
            if (seededRandom(treeA.seed + treeB.seed) < connectChance * 0.1) {
              const startYOffset = (0.2 + seededRandom(treeA.seed + treeB.seed * 2) * 0.6) * treeA.currentGrowth * 30;
              const startX = treeA.x;
              const startY = treeA.y - startYOffset;

              const endYOffset = (0.2 + seededRandom(treeB.seed + treeA.seed * 2) * 0.6) * treeB.currentGrowth * 30;
              const endX = treeB.x;
              const endY = treeB.y - endYOffset;

              const sway = Math.sin(treeA.swayPhase + treeB.swayPhase) * 20 * (dist / connectRadius);
              const perpAngle = Math.atan2(endY - startY, endX - startX) + Math.PI / 2;
              const controlOffset = (seededRandom(treeA.seed * 3) - 0.5) * dist * 0.6;
              
              const controlX = (startX + endX) / 2 + Math.cos(perpAngle) * (controlOffset + sway);
              const controlY = (startY + endY) / 2 + Math.sin(perpAngle) * (controlOffset + sway);
              
              const gradient = ctx.createLinearGradient(startX, startY, endX, endY);
              gradient.addColorStop(0, `rgba(80, 254, 213, 0)`);
              gradient.addColorStop(0.5, `rgba(122, 92, 255, ${0.4 * connectChance})`);
              gradient.addColorStop(1, `rgba(80, 254, 213, 0)`);

              ctx.strokeStyle = gradient;
              ctx.lineWidth = 1 + connectChance;
              ctx.shadowColor = 'rgba(122, 92, 255, 0.7)';
              ctx.shadowBlur = 15;
